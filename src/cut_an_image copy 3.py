...
def _process_line_coords(coords, min_cell_dimension, image_boundary_val, cluster_min_separation=20): # Increased cluster_sep
    """
    ...
    if not final_grid_lines or image_boundary_val - final_grid_lines[-1] >= min_cell_dimension:
        if image_boundary_val not in final_grid_lines: # Avoid adding if already present
            # Check if adding boundary creates a valid cell with the last line
            ...
            elif not final_grid_lines and image_boundary_val >=min_cell_dimension: # No lines yet, add boundary if valid
                 final_grid_lines.append(image_boundary_val)

    elif len(final_grid_lines) > 1 and image_boundary_val > final_grid_lines[-1] : 
        # If boundary is too close but larger, replace last line with boundary if it forms a valid cell
    ...
    if final_grid_lines == [0] and image_boundary_val == 0: # Handle empty image case
        return []
        
    return sorted(list(set(final_grid_lines))) # Ensure unique and sorted


def split_image_into_cells(image_path, output_dir="output_cells_tuned_v2"): # Changed output dir
    """
    Detects grid lines in an image using adaptive thresholding and saves individual cells.
    Parameters tuned to be much more selective, aiming for complex images like 'warrior'.
    """
    ...
    adaptive_thresh_block_size = 31 # Odd number. Increased. Try 25, 31, 35...
    adaptive_thresh_C = 12       # Constant subtracted. Increased. Try values from 10 to 20. Higher C = stricter.
    # Canny Edge Detection
    canny_low_thresh = 60       # Slightly increased low threshold
    canny_high_thresh = 180     # Slightly increased high threshold
    # Hough Line Transform (HoughLinesP)
    hough_rho = 1  
    hough_theta = np.pi / 180  
    hough_threshold = 180       # CRITICAL: Min votes. Increased significantly. Try 150, 180, 200, 250...
    # hough_min_line_length is dynamic
    hough_max_line_gap = 25     # Max gap. If lines are very solid, can be reduced.
    # Line classification
    max_angle_deviation_deg = 5.0 
    # Line processing (for _process_line_coords)
    line_cluster_separation = 25 # Cluster lines within this pixel distance. Increased.
    min_cell_dimension = 40      # Minimum width/height for a cell. Increased.

    # --- End Parameters ---

    print("--- Using Parameters (Tuned v2) ---")
    ...
    # Dynamic hough_min_line_length: Aim for lines that are a substantial part of a cell dimension
    hough_min_line_length = int(min(img_width, img_height) * 0.25) # 25% of the smaller dimension
    ...
    if lines is None:
        print("No line segments detected by HoughLinesP. Try adjusting parameters, especially Hough Threshold or Adaptive Thresholding.")
        return
    ...
    if len(processed_horz_lines) < 2 or len(processed_vert_lines) < 2:
        print("Not enough distinct grid lines found to form cells after processing.")
        return

    for i in range(len(processed_horz_lines) - 1):
        y_start = int(processed_horz_lines[i]) # Ensure integer for slicing
        y_end = int(processed_horz_lines[i+1])
        if y_end <= y_start: continue

        for j in range(len(processed_vert_lines) - 1):
            x_start = int(processed_vert_lines[j]) # Ensure integer for slicing
...
if __name__ == "__main__":
    ...
    # image_to_process = user_image_hedgehog # For testing hedgehog with new params
    # -------------------------------------
    ...
    output_directory = "output_cells_tuned_v2" # New output directory
